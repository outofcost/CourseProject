"""Step 3: M1 — base salary regressions.

M1a — pooled OLS (no FE), basic stats + structural CBA/COVID dummies.
M1b — pooled OLS + year FE, basic stats (year FE absorbs CBA/COVID).
M1c — pooled OLS + year FE, combined stats (per-game + advanced summaries).
M1d — player + year FE (within estimator) on combined stats.

All SE clustered at player_id level.
"""
...
from analysis.config import (BASIC_STATS, COMBINED_STATS, COMMON_NO_STRUCT,
                             COMMON_STRUCT, COMMON_WITHIN)
from analysis.utils import combine_models, panel_index, prep_for_models, save_summary


def run(df: pd.DataFrame) -> dict:
    print("\n" + "=" * 72)
    print("STEP 3: M1 base regression")
    print("=" * 72)
    ...
    print(f"\nM1a (pooled): nobs={int(m1a.nobs)}, R²={m1a.rsquared:.4f}")
