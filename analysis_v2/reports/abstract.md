...
Главный новый результат — **количественная иерархия факторов**: Performance + Age = 65.5% объяснённой дисперсии; Awards = 12.2%; Demographics = 14.1%; Durability = 5.7%; Market + Team + Structural суммарно < 3%. Из 11 ключевых гипотез 7 подтверждены при контроле multiple testing (BH-FDR 5%). Salary cap создаёт quasi-deterministic institutional layer (tier dummies одни дают R² = 0.85), модулирующий performance-pricing через discrete category jumps. Market-size премии нет — обнаружен дисконт ~10% для крупных рынков (anti-marketability, согласуется с Hembre 2021). Awards channel работает с 2-3-летней задержкой через contract renewal cycle. Multi-time All-NBA игроки в фазе decline получают penalty −22% — survival bias эффект aging-vet selection.
...
The headline new finding is a **quantitative hierarchy of factors**: Performance + Age = 65.5% of explained variance; Awards = 12.2%; Demographics = 14.1%; Durability = 5.7%; Market + Team + Structural jointly < 3%. Of 11 key hypotheses, 7 survive multiple testing (BH-FDR 5%). Salary cap creates a quasi-deterministic institutional layer (tier dummies alone yield R² = 0.85), modulating performance-pricing via discrete category jumps. No market-size premium found — a ~10% discount detected for large-market teams (anti-marketability, consistent with Hembre 2021). Awards channel operates with 2-3 year lag through contract renewal cycle. Multi-time All-NBA players in decline phase receive −22% penalty — survival-bias effect of aging-vet selection.
