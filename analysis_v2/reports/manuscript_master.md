...
Четвёртый research stream касается влияния external market characteristics — размера телерынка, городского населения, состава медиа — на individual зарплаты. Теоретическое ожидание (Rosen 1981 superstar mechanism) — premium для игроков в крупных media markets через amplified individual market exposure.
...
Theoretical channel для team-side эффекта налогообложения подробно обсуждался в §2.5 (Hembre, 2022); здесь повторно фиксируется только наблюдение, что Hembre pooled-sport effect (1pp ↑ state tax → −0.77 до −0.86 pp team win percentage) с NBA-specific coefficient статистически незначимым (Hembre, 2022, Table 3, p. 20) даёт theoretical channel для нашего anti-marketability finding, но не direct empirical confirmation на уровне NBA.

Совокупно: Kleven + Alm + Johnson-Hall + Hembre дают сильное theoretical + cross-sport empirical evidence существования tax channel в sports labor markets. Наш H9 null в post-2017 NBA — informative deviation, требующая институциональной интерпретации (cap-binds, restricted mobility, off-court endorsement substitution).
...
![**Рисунок F2.** Возрастной профиль $\ln(\text{salary})$ по спецификации M1c. Точки — bin-medians, тёмная линия — fitted quadratic с 95% CI; вертикальная штриховая линия — peak age 29.5.](../output/figures/F2_age_profile.png){#fig:F2 width=85%}
...
![**Рисунок F3.** Tier-specific Mincer-эффект очков-за-игру: $\beta_{\text{ppg}}$ внутри каждого contract tier. Cap-induced concavity видна как монотонное снижение от mid_level к max_30.](../output/figures/F3_tier_beta_ppg.png){#fig:F3 width=80%}

![**Рисунок F5.** Boxplot $\ln(\text{salary})$ по 8 contract tiers. Discrete jumps между tiers значительно больше внутри-tier разброса — визуальное доказательство step-function structure NBA-зарплаты.](../output/figures/F5_tier_boxplot.png){#fig:F5 width=85%}
...
![**Рисунок F4.** Event study вокруг первого All-NBA selection: коэффициенты $\beta_\tau$ относительно $\tau = -1$ (reference period), с 95% CI. Значимые положительные эффекты при $\tau = +2$ (+21%) и $\tau = +3$ (+22%); pre-trends чисты.](../output/figures/F4_event_study_all_nba.png){#fig:F4 width=85%}
...
Эффективность Шепли (efficiency axiom): $|\sum_i \phi_i - R^2_{\text{full}}| = 1.4 \times 10^{-16}$ (машинная точность).

![**Рисунок F1.** Waterfall-декомпозиция $R^2$ методом Шепли по 9 тематическим блокам факторов. Главный методологический результат работы: Performance + Age совокупно дают 65.5% объяснённой дисперсии $\ln(\text{salary})$, оставшиеся блоки распределяются от Demographics (14.1%) до Market (0.2%).](../output/figures/F1_waterfall_shapley.png){#fig:F1 width=90%}
...
Sequential R² показывает большую чувствительность к порядку: атрибуция блока Awards меняется в 33 раза между plan- и reverse-order (0.006 → 0.199). Shapley даёт устойчивую оценку 0.079.

![**Рисунок F7.** Сравнение Sequential R² (plan-order и reverse-order) с Shapley-декомпозицией по 9 блокам. Большой разрыв plan vs reverse для Awards и Performance демонстрирует order-dependence sequential-метода; Shapley даёт единственную axiom-обоснованную атрибуцию.](../output/figures/F7_seq_vs_shapley.png){#fig:F7 width=85%}
...
![**Рисунок F6.** Forest plot главных эффектов спецификации M_full (n = 2 268) с 95% CI на cluster-robust SE по `player_id`. Tier-эффекты доминируют по магнитуде; awards-каналы и durability формируют значимый второй уровень; market- и team-эффекты пересекают ноль.](../output/figures/F6_forest_M_full.png){#fig:F6 width=90%}
