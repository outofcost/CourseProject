"""Robustness V1 — R1-R5 на новом cy.

V1 vs V0:
    R3 (excl. rookies): теперь experience=0 не существует (после A2 fix),
        поэтому условие меняется на experience > 1 (исключить первый сезон).
        Это исключит ~430 «настоящих» новичков, а не 612 undrafted.
    R4 (alt cy = team only): теперь cy_B_offseason (без mid-season trade).
    R5 (alt cy = salary only): теперь cy_A_up (без pay-cut).
"""
from __future__ import annotations

import sys
from pathlib import Path

import pandas as pd
import statsmodels.api as sm
from linearmodels.panel import PanelOLS

ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT))
from analysis.config import COMBINED_STATS, COMMON_NO_STRUCT  # noqa: E402
from analysis_v1.utils_v1 import (combine_models, panel_index,  # noqa: E402
                                  prep_for_models, save_summary)


def run(df: pd.DataFrame) -> dict:
    print("\n" + "=" * 72)
    print("STEP 6 (V1): robustness checks")
    print("=" * 72)

    d = prep_for_models(df, restrict_single_team=True)
    d = d.dropna(subset=["state_tax_rate", "no_income_tax", "contract_year"])
    d["contract_year"] = d["contract_year"].astype(int)

    base_cols = COMBINED_STATS + COMMON_NO_STRUCT

    # ── R1 full ─────────────────────────────────────────────────────────
    d_idx = panel_index(d)
    y = d_idx["ln_salary"]
    X = sm.add_constant(d_idx[base_cols + ["no_income_tax", "contract_year"]])
    r1 = PanelOLS(y, X, time_effects=True, check_rank=False).fit(
        cov_type="clustered", cluster_entity=True
    )
    save_summary(r1, "R1_full_tax_plus_contract")

    # ── R2 sub-$1M dropped ───────────────────────────────────────────────
    d_high = d[d["salary_usd"] >= 1_000_000].copy()
    d_high_idx = panel_index(d_high)
    y_high = d_high_idx["ln_salary"]
    X_high = sm.add_constant(
        d_high_idx[base_cols + ["no_income_tax", "contract_year"]]
    )
    r2 = PanelOLS(y_high, X_high, time_effects=True, check_rank=False).fit(
        cov_type="clustered", cluster_entity=True
    )
    save_summary(r2, "R2_excl_minimum_contracts")
    print(f"R2: dropped {(d['salary_usd'] < 1_000_000).sum()} sub-$1M rows")

    # ── R3 excl. first-year (experience > 1) ─────────────────────────────
    # После A2 fix exp=0 не существует; «новичок» = experience == 1.
    d_vet = d[d["experience"] > 1].copy()
    d_vet_idx = panel_index(d_vet)
    y_vet = d_vet_idx["ln_salary"]
    # undrafted больше не collinear с experience>1, оставляем регрессор.
    X_vet = sm.add_constant(
        d_vet_idx[base_cols + ["no_income_tax", "contract_year"]]
    )
    r3 = PanelOLS(y_vet, X_vet, time_effects=True, check_rank=False).fit(
        cov_type="clustered", cluster_entity=True
    )
    save_summary(r3, "R3_excl_first_year")
    print(f"R3: dropped {(d['experience'] == 1).sum()} first-year rows")

    # ── R4 alt cy: cy_B_offseason ────────────────────────────────────────
    d_alt = d.copy()
    d_alt["cy_team_only"] = (d_alt["cy_B_offseason"].fillna(0) == 1).astype(int)
    d_alt_idx = panel_index(d_alt)
    y_alt = d_alt_idx["ln_salary"]
    X_alt = sm.add_constant(
        d_alt_idx[base_cols + ["no_income_tax", "cy_team_only"]]
    )
    r4 = PanelOLS(y_alt, X_alt, time_effects=True, check_rank=False).fit(
        cov_type="clustered", cluster_entity=True
    )
    save_summary(r4, "R4_alt_cy_B_offseason")

    # ── R5 alt cy: cy_A_up ───────────────────────────────────────────────
    d_alt2 = d.copy()
    d_alt2["cy_salary_only"] = (d_alt2["cy_A_up"].fillna(0) == 1).astype(int)
    d_alt2_idx = panel_index(d_alt2)
    y_alt2 = d_alt2_idx["ln_salary"]
    X_alt2 = sm.add_constant(
        d_alt2_idx[base_cols + ["no_income_tax", "cy_salary_only"]]
    )
    r5 = PanelOLS(y_alt2, X_alt2, time_effects=True, check_rank=False).fit(
        cov_type="clustered", cluster_entity=True
    )
    save_summary(r5, "R5_alt_cy_A_up")

    combine_models(
        {"R1 (full)": r1,
         "R2 (excl <$1M)": r2,
         "R3 (excl first-year)": r3,
         "R4 (cy_B_offseason)": r4,
         "R5 (cy_A_up)": r5},
        "R_robustness.csv",
    )

    for label, model in [("R1", r1), ("R2", r2), ("R3", r3),
                         ("R4", r4), ("R5", r5)]:
        print(f"{label}: nobs={int(model.nobs)}, R²={model.rsquared:.4f}")
    return {"R1": r1, "R2": r2, "R3": r3, "R4": r4, "R5": r5}
